...
export default function MultiRegionWebsiteDetailsPage() {
  ...
  const fetchWebsiteInfo = async () => {
    ...
    try {
      const [response1, response2] = await Promise.all([
        axios.get(`${BACKEND_URL}/website/status/${websiteId}`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        }),
        axios.get(`${BACKEND_URL}/region/regions-with-name`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        }),
      ...
      const processedData = processRegionalData(
        response1.data.website,
        response2.data.regions
      );
  ...
  const processRegionalData = (data: any, allRegions: IRegion[]): IWebsite => {
    ...
    const REGION_NAMES: { [key: string]: string } = allRegions.reduce(
      (acc, curr: IRegion) => ({ ...acc, [curr.id]: curr.name }),
      {}
    );

    const regions: IRegionData[] = Array.from(regionMap.entries()).map(
      ([regionId, ticks]) => {
        const upTicks = ticks.filter((tick) => tick.status === "UP");
        const uptime =
          ticks.length > 0 ? (upTicks.length / ticks.length) * 100 : 0;
        const avgResponseTime =
          upTicks.length > 0
            ? Math.round(
                upTicks.reduce((sum, tick) => sum + tick.reponseTimeMs, 0) /
                  upTicks.length
              )
            : 0;
    ...
    const totalTicks = regions.reduce(
      (sum, region) => sum + region.ticks.length,
      0
    );
    const totalUpTicks = regions.reduce(
      (sum, region) =>
        sum + region.ticks.filter((tick) => tick.status === "UP").length,
      0
    );
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <Card className="bg-slate-800/50 backdrop-blur-xl border-slate-700/50 hover:bg-slate-800/70 transition-all duration-200 group">
              ...
              <CardContent>
                <div className="text-2xl font-bold text-emerald-400">
                  {website?.globalUptime.toFixed(1)}%
                </div>
            ...
            <Card className="bg-slate-800/50 backdrop-blur-xl border-slate-700/50 hover:bg-slate-800/70 transition-all duration-200 group">
              ...
              <CardContent>
                <div className="text-2xl font-bold text-emerald-400">
                  {website?.regions.filter((r) => r.currentStatus === "UP")
                    .length || 0}
                  /{website?.regions.length || 0}
                </div>
                <p className="text-xs text-slate-400 flex items-center mt-1">
          ...
          <Card className="bg-slate-800/50 backdrop-blur-xl border-slate-700/50 mb-8">
            <CardHeader>
              ...
              <CardDescription className="text-slate-400">
                Current status and performance metrics for each monitoring
                region
              </CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    <CardContent className="pt-0">
                      <div className="space-y-2">
                        <div className="flex justify-between items-center">
                          ...
                          <span
                            className={`text-sm font-bold ${getRegionHealthColor(region.uptime)}`}
                          >
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-xs text-slate-300">
                            {formatDistance(region.lastChecked, new Date(), {
                              addSuffix: true,
                            })}
                          </span>
                        </div>
                              ...
                              {region.ticks
                                ...
                                .map((tick, index) => {
                                  const height =
                                    tick.status === "DOWN"
                                      ? 4
                                      : (tick.reponseTimeMs / maxResponseTime) *
                                          40 +
                                        8;
                                  return (
                                    <div
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <Card className="bg-slate-800/50 backdrop-blur-xl border-slate-700/50">
                    ...
                    <CardContent>
                      <div
                        className={`text-2xl font-bold ${getRegionHealthColor(region.uptime)}`}
                      >
                        {region.uptime.toFixed(1)}%
                      </div>
                      <p className="text-xs text-slate-400 flex items-center mt-1">
